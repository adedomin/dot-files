function color_hash --description "Hash given string with a color, based on sdbm."
    ...
    set --global hash_red (math --base=16 --scale=0 $hash / 'pow(2,16)' % 'pow(2,8)')
    set --global hash_green (math --base=16 --scale=0 $hash / 'pow(2,8)' % 'pow(2,8)')
    set --global hash_blue (math --base=16 --scale=0 $hash % 'pow(2,8)')
